<?php  if(!defined('_source')) die("Error");	
	...
	$d->reset();
	$sql = "select id,ten from #_httt order by stt asc";
	$d->query($sql);
	...
	if(!empty($_POST)){	
		...
		{	
			...
			//dump($tonggia);
			$sql = "INSERT INTO  table_donhang (madonhang,hoten,dienthoai,diachi,email,httt,phithem,tonggia,thanhpho,quan,phuong,noidung,ngaytao,tinhtrang,ngaycapnhat,ip,id_user) 
			  VALUES ('$madonhang','$hoten','$dienthoai','$diachi','$email','$httt','$phiship','$tongtien_phiship','$thanhpho','$quan','$phuong','$noidung','$ngaydangky','1','$ngaycapnhat','$ip','$id_user')";	
		...
		if(mysql_query($sql))
		{
			if(is_array($_SESSION['cart']))
			{
				...
				for($i=0;$i<$max;$i++){
					...
					//=== end id don hang===
					$sql = "INSERT INTO table_chitietdonhang (madonhang,masp,ten,size,mausac,gia,soluong,tonggia,ngaytao,photo,id_donhang,id_sanpham) VALUES ('$madonhang','$pmasp','$pname','$size','$mausac','$pgia','$q','$ptonggia','$ngaydangky','$pphoto','$id_donhang','$pid')";
				...
				{		
					...
					*=====================================*/
					...
					$mail->CharSet = "utf-8";		
						...
						$body .= '
							...
							<tr>
								<th>Phí ship :</th><td>'.number_format($phiship,0, ',', '.').' VNĐ'.'</td>
							</tr>
							<tr>
								<th>Tổng tiền :</th><td>'.number_format($ptonggia,0, ',', '.').' VNĐ'.'</td>
							</tr>
							<tr>
								<th>Tổng thanh toán :</th><td>'.number_format($tongtien_phiship,0, ',', '.').' VNĐ'.'</td>
							</tr>
							
							<tr>
								<th>Ghi Chú :</th><td>'.$noidung.'</td>
							</tr>
							';
						$body .= '</table>';
						...
						if(is_array($_SESSION['cart']))
						{
							$body.= '<tr bgcolor="#f5f5f5" height="30px">
								...
								<td align="center" class="gh_an">Tổng Giá</td>
							</tr>';
							$max=count($_SESSION['cart']);
							for($i=0;$i<$max;$i++){
								$pid=$_SESSION['cart'][$i]['productid'];
								...
								$body.='<td width="10%" align="center" class="gh_an">'.number_format(get_price($pid)*$q,0, ',', '.') .'&nbsp;VNĐ</td>
								</tr>';
							}
							$body.='<tr bgcolor="#f5f5f5" height="30px"><td colspan="7">PHÍ SHIP: '.number_format($phiship,0, ',', '.').'&nbsp; VNĐ</td></tr>';
							$body.='<tr bgcolor="#f5f5f5" height="30px"><td colspan="7">TỔNG GIÁ: '.number_format(get_order_total(),0, ',', '.').'&nbsp; VNĐ</td></tr>';
							$body.='<tr bgcolor="#f5f5f5" height="30px"><td colspan="7">TỔNG THANH TOÁN: '.number_format($tongtien_phiship,0, ',', '.').'&nbsp; VNĐ</td></tr>';
						}
						else{
				   ...
				   #------------Chi tiết đơn hàng---------------------
						...
						if($mail->Send())
						{
							...
							transfer("Bạn đã đặt hàng thành công.<br> Chúng tôi sẽ liên hệ với bạn sớm nhất.<br>Mã đơn hàng là: ".$madonhang, "index.html");
						}
						else
							transfer("Bạn đã đặt hàng thành công.<br> Chúng tôi sẽ liên hệ với bạn sớm nhất.<br>Mã đơn hàng là: ".$madonhang, "index.html");
						}
