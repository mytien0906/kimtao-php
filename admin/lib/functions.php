...
function get_pricekm($pid){ // tinhs phaan tram
	global $d, $row,$kq;

	$sql = "select gia,giacu from table_product where id=$pid";
	$d->query($sql);
	$row = $d->fetch_array();
	$giagiam= $row['giacu'] - $row['gia'];
	$phantram=round(($giagiam/$row['giacu'])*100);
	$kq= '<div class="giamgia">'.$phantram.'%</div>';
