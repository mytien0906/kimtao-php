...
function save_item(){
	...
	if($id){
		...
		$d->setWhere('id', $id);
		if($d->update($data)){
			/*--- ca nhat so luong san pham----*/
				
			if($_POST['id_tinhtrang']==4){
				$d->reset();
				$sql = "select soluong,id_sanpham,id_donhang from table_chitietdonhang where id_donhang='".$id."'";
				$d->query($sql);
				$kq_1 = $d->result_array();
				for($i=0;$i<count($kq_1);$i++){
					$d->reset();
					$sql = "select banra,tonkho,soluong from table_product where id='".$kq_1[$i]['id_sanpham']."'";
					$d->query($sql);
					$soluong_sp = $d->fetch_array();
					
					$soluong_banra_cuoi = $kq_1[$i]['soluong'] + $soluong_sp['banra'];
					$soluongtonkho = $soluong_sp['soluong'] - $soluong_banra_cuoi;
					
					$sql_capnhat = "UPDATE table_product 
								SET banra='".$soluong_banra_cuoi."',tonkho='".$soluongtonkho."' 
								WHERE id='".$kq_1[$i]['id_sanpham']."' ";
