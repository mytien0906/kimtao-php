...
function get_items(){
	...
	$sql = "select * from #_bando where id<>0 $where limit $bg,$pageSize";		
	$d->query($sql);
...
function get_item(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=bando&act=man".$urlcu);
	
	$sql = "select * from #_bando where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực","index.php?com=bando&act=man".$urlcu);
	$item = $d->fetch_array();
}
//===========================================================
function save_item(){
	...
	if($id){
		...
		if($d->update($data))			
			...
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=bando&act=man".$urlcu);
	}else{
		...
		if($d->insert($data))
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=bando&act=man".$urlcu);
	}
}
//===========================================================
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=bando&act=man".$urlcu);
		...
		} redirect("index.php?com=bando&act=man".$urlcu);} else transfer("Không nhận được dữ liệu", "index.php?com=bando&act=man".$urlcu);
}
