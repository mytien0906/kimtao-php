<script language="javascript" type="text/javascript">
	...
	$("#xoahet").click(function(){
		...
		if (hoi==true) document.location = "index.php?com=baihat&act=delete&listid=" + listid;
	});
	});
	...
	{	
		...
		window.location ="index.php?com=baihat&act=man&key="+a;	
		return true;
	}
</script>
...
<div class="control_frm" style="margin-top:0;">
  	<div style="float:left;">
    	<input type="button" class="blueB" value="Thêm" onclick="location.href='index.php?com=baihat&act=add'" />
 ...
 <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
	...
	<tbody>
     <?php for($i=0, $count=count($items); $i<$count; $i++){?>
        ...
        <td>
            <a href="index.php?com=baihat&act=edit&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" class="tipS SC_bold">
             <?=$items[$i]['ten']?>
            </a>
        </td>
        ...
        <td class="actBtns">
            <a href="index.php?com=baihat&act=edit&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" title="" class="smallButton tipS" original-title="Sửa danh mục"><img src="./images/icons/dark/pencil.png" alt=""></a>
           <a href="index.php?com=baihat&act=delete&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" onClick="if(!confirm('Xác nhận xóa <?=$items[$i]['ten']?>')) return false;" title="" class="smallButton tipS" original-title="Xóa bài viết"><img src="./images/icons/dark/close.png" alt=""></a> 
        </td>
      </tr>
           <?php } ?> 
    </tbody>
