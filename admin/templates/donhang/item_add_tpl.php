...
function tinhtrang($i=0)
	{
		...
		$str='
			<select id="id_tinhtrang" name="id_tinhtrang" class="main_select select_danhmuc">					
			';
...
function get_httt()
	{
		...
		$stmt=mysql_query($sql);
		$str='
			<select id="httt" name="httt" class="main_select select_danhmuc">
...
<form name="supplier" id="validate" class="form" action="index.php?com=order&act=save" method="post" enctype="multipart/form-data">
	<div class="widget">
        ...
        <div class="formRow">
			<label>Địa chỉ</label>
			<div class="formRight">
             <input type="text" name="diachi" title="Địa chỉ khách hàng" style="width:30% !important" readonly="readonly" id="diachi" class="tipS validate[required] read" value="<?=@$item['diachi']?>" /><?=get_thanhpho_item();?><?=get_thanhpho();?><?=get_phuong();?>
			</div>
			<div class="clear"></div>
		</div>	
        ...
        <div class="formRow">
			...
			<div class="formRight">
             <input type="text" name="phithem" title="Cộng thêm phí giao hàng hoặc - khuyến mãi cho khách hàng(Nếu khuyến mãi khách hàng thì thêm dấu -)"  id="phithem" class="tipS" value="<?=@$item['phithem']?>" />
			</div>
			<div class="clear"></div>
		</div>	        
    ...
    <thead>
      <tr>
        ...
        <?php /*?><td width="100">Giá cũ</td><?php */?>
    ...
    <tbody>
        ...
        <tr id="productct<?=$chitietdonhang[$i]['id']?>">
          ...
          <?php /*?><td align="center"><?=number_format($chitietdonhang[$i]['giacu'],0, ',', '.')?>&nbsp;VNĐ</td><?php */?>
          <td align="center">
