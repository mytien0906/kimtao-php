...
function show_image ($mime_type, $image_resized, $quality, $cache_dir, $new_width) {
	...
	$d->reset();
	$sql_dongdau = "select photo,thumb from #_background where type='dongdau' limit 0,1";
	$d->query($sql_dongdau);
	$dongdau = $d->fetch_array();
	//print_r($dongdau);

	//$watermark = imagecreatefromfile("upload/hinhanh/".$dongdau['dongdau']);
	...
	if($w_hinh > 400)
	  {
		  $watermark_img = 'upload/hinhanh/'.$dongdau['dongdau'];
	  }
	else
	  {
		  $watermark_img = 'upload/hinhanh/'.$dongdau['thumb'];
	  }
